...
4. **Mathematical & Spatial Bounds:** When scaling difficulty by increasing object count or UI elements, always calculate the maximum possible spatial footprint. Ensure all UI elements fit on the screen. Ensure the total area of generated puzzle pieces/enemies does not exceed the literal grid capacity. Implement strict limits or bounds-checking to prevent unbeatable states and visual overflow.
5. **Game Over / Defeat Screens:** NEVER route a player directly back to the "Start" menu or main menu upon death or failing a level. Always create a dedicated `scene("lose")` or `scene("gameover")` that displays their final score/level, a "Game Over" message, and a button to return to the main menu. 
